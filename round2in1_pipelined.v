...
// 2-stage pipelined double-round keccak
// Stage 1: in  -> Round 1 (theta+rho+pi+chi+iota with round_const_1) -> g1 -> g_reg
// Stage 2: g_reg -> Round 2 (theta+rho+pi+chi+iota with round_const_2) -> gg -> out
module round2in1_pipelined (
    input  wire         clk,
    input  wire         en,              // pipeline enable (assert when feeding/advancing)
    input  wire [1599:0] in,
    input  wire [63:0]   round_const_1,
    input  wire [63:0]   round_const_2,
    ...
    wire   [63:0]   e[4:0][4:0];
    wire   [63:0]   f[4:0][4:0];
    wire   [63:0]   g1[4:0][4:0];   // first round output (before pipeline reg)

    /* pipeline registers between round 1 and 2 */
    reg    [63:0]   g_reg[4:0][4:0];
    ...
    // -------------------------------------------------------------------------
    // ROUND 1 (COMBINATIONAL) : in -> a -> b -> c -> d -> e -> f -> g1
    // -------------------------------------------------------------------------

    /* assign "a[x][y] == in[w(5y+x)+z]" */
    generate
      for (y = 0; y < 5; y = y + 1) begin : R1_L0
        for (x = 0; x < 5; x = x + 1) begin : R1_L1
    ...
    /* calc "c == theta(a)" */
    generate
      for (y = 0; y < 5; y = y + 1) begin : R1_L3
        for (x = 0; x < 5; x = x + 1) begin : R1_L4
    ...
    /* calc "f = chi(e)" */
    generate
      for (y = 0; y < 5; y = y + 1) begin : R1_L5
        for (x = 0; x < 5; x = x + 1) begin : R1_L6
          assign f[x][y] = e[x][y] ^ ((~ e[`add_1(x)][y]) & e[`add_2(x)][y]);
        end
      end
    endgenerate

    /* calc "g1 = iota(f)" for round 1 */
    generate
      for (x = 0; x < 64; x = x + 1) begin : R1_L60
        if (x==0 || x==1 || x==3 || x==7 || x==15 || x==31 || x==63)
          assign g1[0][0][x] = f[0][0][x] ^ round_const_1[x];
        else
          assign g1[0][0][x] = f[0][0][x];
      end
    endgenerate

    generate
      for (y = 0; y < 5; y = y + 1) begin : R1_L7
        for (x = 0; x < 5; x = x + 1) begin : R1_L8
          if (x!=0 || y!=0)
            assign g1[x][y] = f[x][y];
        end
      end
    ...
    // -------------------------------------------------------------------------
    // PIPELINE REGISTER BETWEEN ROUND 1 AND 2
    // -------------------------------------------------------------------------
    always @(posedge clk) begin
      if (en) begin
        for (iy = 0; iy < 5; iy = iy + 1) begin
          for (ix = 0; ix < 5; ix = ix + 1) begin
            g_reg[ix][iy] <= g1[ix][iy];
          end
        end
      end
    end
    ...
    // -------------------------------------------------------------------------
    // ROUND 2 (COMBINATIONAL) : g_reg -> bb/cc/dd/ee/ff/gg -> out
    // -------------------------------------------------------------------------

    /* calc "bb[x] == g_reg[x][0] ^ g_reg[x][1] ^ ... ^ g_reg[x][4]" */
    generate
      for (x = 0; x < 5; x = x + 1) begin : R2_L12
        assign bb[x] = g_reg[x][0] ^ g_reg[x][1] ^ g_reg[x][2] ^ g_reg[x][3] ^ g_reg[x][4];
      end
    endgenerate

    /* calc "cc == theta(g_reg)" */
    generate
      for (y = 0; y < 5; y = y + 1) begin : R2_L13
        for (x = 0; x < 5; x = x + 1) begin : R2_L14
          assign cc[x][y] = g_reg[x][y] ^ bb[`sub_1(x)] ^ `rot_up_1(bb[`add_1(x)]);
        end
      end
    endgenerate
    ...
    /* calc "ff = chi(ee)" */
    generate
      for (y = 0; y < 5; y = y + 1) begin : R2_L15
        for (x = 0; x < 5; x = x + 1) begin : R2_L16
          assign ff[x][y] = ee[x][y] ^ ((~ ee[`add_1(x)][y]) & ee[`add_2(x)][y]);
        end
      end
    endgenerate

    /* calc "gg = iota(ff)" for round 2 */
    generate
      for (x = 0; x < 64; x = x + 1) begin : R2_L160
        if (x==0 || x==1 || x==3 || x==7 || x==15 || x==31 || x==63)
    ...
    generate
      for (y = 0; y < 5; y = y + 1) begin : R2_L17
        for (x = 0; x < 5; x = x + 1) begin : R2_L18
          if (x!=0 || y!=0)
