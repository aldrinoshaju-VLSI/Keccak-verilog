...
module keccak #(
    ...
    reg                state;     /* 0: more input; 1: last seen */
    ...
    wire      [1599:0] f_out;
    wire               f_out_ready;
    wire      [OUTPUT_SIZE-1:0]  out1;      /* before reorder byte */
    reg       [10:0]   i;         /* gen "out_ready" for 12 two-round steps */
    genvar w, b;
    ...
    /* reorder byte per 64-bit word for 512-bit output (8 words) */
    generate
    ...
    /* reorder byte per 64-bit word for rate-sized padder block */
    generate
    ...
    /* padder: pass rate & suffix (SHAKE uses SUFFIX=8'h1F) */
    padder_fast_2 #(
      .RATE_BITS(RATE_BITS),
      .SUFFIX   (SUFFIX)
    ) padder_ (
    ...
    /* permutation: two rounds per step; just pass rate for absorb splice */
    f_permutation #(
      .RATE_BITS(RATE_BITS)
    ) f_permutation_ (
      .clk(clk), .reset(reset),
